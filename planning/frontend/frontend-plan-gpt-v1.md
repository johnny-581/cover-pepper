Awesome — here’s a complete, minimal (black & white) Vite + React + TypeScript frontend that matches your backend and UI. It uses Tailwind, Monaco, TanStack Query, Zustand, axios, React Router, and a tiny custom modal/context-menu. The “Generate” dialog has **no template picker** (it uses the current template set via right-click). Account + Settings icons live at the **bottom** of the directory panel.

I’ve included every file you need. You can copy-paste into a fresh Vite app.
...
export default function AppLayout() {
  ...
  // Ensure /app/letters routes mount something even when empty
  useEffect(() => {
    // If needed, we could auto-select the first letter here.
  }, [data, navigate]);
...
export function useAuth() {
  ...
  const logout = useMutation({
    ...
    onSuccess: () => {
      qc.clear();
      // browser will re-render router -> LoginPage
    }
  });
...
export default function Modal({ open, onClose, title, widthClass, children }: PropsWithChildren<Props>) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/20 flex items-center justify-center" onClick={onClose}>
      <div
        className={clsx("bg-paper border border-grayline p-4", widthClass ?? "w-[520px]")}
...
## src/features/letters/components/ContextMenu.tsx
...
## src/features/letters/components/DirectoryPanel.tsx
    ...
    <aside className="w-[280px] min-w-[240px] max-w-[320px] border-r border-grayline flex flex-col">
      <div className="p-3 border-b border-grayline">
        <div className="flex gap-2">
          <button className="flex-1 border border-black px-2 py-1 text-sm" onClick={() => setGenerateOpen(true)}>
            New letter
          </button>
          <button className="flex-1 border border-black px-2 py-1 text-sm" onClick={() => setUploadOpen(true)}>
            Upload
          </button>
        </div>
      </div>
...
## src/features/letters/components/LetterList.tsx
  ...
  if (!letters?.length) {
    return (
      <div className="p-3 text-sm text-gray-600">
        No letters yet. Upload or generate one.
      </div>
    );
  }
...
## src/features/letters/components/LetterListItem.tsx
...
export default function LetterListItem({ letter }: { letter: Letter }) {
  ...
  const items = [
    ...
    },
    {
      ...
      onClick: () => setTemplateLetterId(letter.id)
    },
    {
      label: "Delete",
      onClick: async () => {
        ...
        // If deleted current letter, navigate to another or list root
        if (selected) {
...
## src/features/letters/components/LettersPage.tsx
...
export default function LettersPage() {
  ...
  const { isGenerateOpen, isUploadOpen, setGenerateOpen, setUploadOpen } = useUI();
...
## src/features/letters/components/EditorPanel.tsx
...
export default function EditorPanel({ letterId, letter }: { letterId: string; letter: Letter | null }) {
  if (!letter) {
    return <div className="p-4 text-sm">Loading letter…</div>;
  }
...
## src/features/letters/components/LetterToolbar.tsx
...
export default function LetterToolbar({ letter }: { letter: Letter }) {
  ...
  const handleCompile = async () => {
    // Ensure pending title save flushed
...
export default function LetterEditor({ letter }: { letter: Letter }) {
  ...
  const onMount: OnMount = (editor, monaco) => {
    // Minimal LaTeX syntax: register once
...
## src/features/letters/components/dialogs/GenerateDialog.tsx
...
export default function GenerateDialog({ open, onClose }: Props) {
  ...
  const { id } = useParams();
  const { data: current } = useLetter(id);
  ...
  const handleSubmit = async () => {
    ...
    // Fetch template latex (always up-to-date)
...
## src/features/letters/components/dialogs/UploadDialog.tsx
...
export default function UploadDialog({ open, onClose }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Upload LaTeX Letter">
...
```bash
npm create vite@latest cover-pepper -- --template react-ts
cd cover-pepper
```

2. Replace files with the ones above, then:
...
* **Generate dialog** uses the **current template** (set via right-click → “Set as template”). No template picker is shown.
* **Compile** downloads a PDF named after the current title.
* **Autosave**: LaTeX body saves after \~800ms idle; `Cmd/Ctrl+S` saves immediately.
* **Title** saves after \~500ms; compile flushes pending saves first.
* **Account + Settings** icons are at the **bottom** of the directory panel; logout is a confirm action.
* Minimal, black-and-white Tailwind; no extra styling beyond borders/spacing.

If you’d like, I can add lightweight virtualization for long lists, or wire a real settings panel (e.g., theme, editor options) later.
