...
export default function UploadDialog({ open, onClose }: Props) {
  ...
  const hadNoLetters = (data?.length ?? 0) === 0;
  ...
  const handleSubmit = async () => {
    ...
    onClose();
    router.push(`/app/letters/${created.id}`);
    router.refresh();
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title="Upload">
      ...
      <div className="flex items-center gap-2">
        ...
        {fileName && <span className="text-gray">{fileName}</span>}
      </div>
      ...
      <div className="flex items-center justify-between mt-5">
        {error ? <div className="text-theme-red">{error}</div> : <div />}
        <div className="flex items-center gap-2">
